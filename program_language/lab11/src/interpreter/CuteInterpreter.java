...
public class CuteInterpreter {
    public static void main(String[] args) throws IOException {
        List<HashMap<String, Node>> archive = new ArrayList<>();//id 노드에 대한 값을 저장 해야 하는 곳
        archive.add(new HashMap<>());//define 전용의 HashMap
        while (true) {
            System.out.print("> ");
    ...
    private List<HashMap<String, Node>> archive;// array List nested 정도를 나타내는 자료 구조 index가 커지면 깊어진다

    public CuteInterpreter(List<HashMap<String, Node>> archive) {
        this.archive = archive;
    }

    private ListNode valueDataOfLambda;

    private void insertNestedVariable(HashMap<String, Node> newHashMap, ListNode variable) {
        if (valueDataOfLambda.car() == null) {
            return;
        }
        if (variable.car() == null) {
            this.valueDataOfLambda = null;//변수 초기화 후 전부 hashMap arrayList등록
            return;
        }
        Node nodeData = valueDataOfLambda.car();
        if (nodeData instanceof IdNode) {
            nodeData = findHashMapData(this.archive.size() - 1, (IdNode) nodeData);
        }
        newHashMap.put(variable.car().toString(), this.runExpr(nodeData));
        valueDataOfLambda = valueDataOfLambda.cdr();
        this.insertNestedVariable(newHashMap, variable.cdr());
    }

    private Node findHashMapData(int index, IdNode id) {//nested 정도의 값을 가져옴
        HashMap<String, Node> nowIndexArchive = this.archive.get(index);
        if (index == 0) {
            return nowIndexArchive.get(id.toString());
        } else {
            Node resultNode = nowIndexArchive.get(id.toString());
            if (resultNode == null) {
                return this.findHashMapData(index - 1, id);
            } else {
                return resultNode;
            }
        }
    ...
    private void insertTable(IdNode id, Node value) {//define 전용
        if (this.archive.get(this.archive.size() - 1).containsKey(id.toString())) {
            this.archive.get(this.archive.size() - 1).replace(id.toString(), value);
        } else {
            this.archive.get(this.archive.size() - 1).put(id.toString(), value);
        }
    }
    ...
    private Node runList(ListNode list) {//list node에 대해 해당 노드에 대해 runExpr를 호출
        if (list.car() instanceof IdNode) {
            Node lambdaList = list;
            ListNode lambdaListCdr = list.cdr();
            lambdaList = this.findHashMapData(this.archive.size() - 1, (IdNode) list.car());
            return this.runList(ListNode.cons(lambdaList, lambdaListCdr));
        }
        ...
        if (list.car() instanceof FunctionNode) {
            if (this.valueDataOfLambda == null && ((FunctionNode) list.car()).funcType == FunctionNode.FunctionType.LAMBDA) {
                return list;
            }
        ...
        if (list.car() instanceof ListNode && ((ListNode) list.car()).car() instanceof FunctionNode
                && ((FunctionNode) ((ListNode) list.car()).car()).funcType == FunctionNode.FunctionType.LAMBDA) {
            this.valueDataOfLambda = list.cdr();
            return this.runList((ListNode) list.car());
        }
        return list;
    }
        ...
        switch (operator.funcType) {
            case CAR:
                ...
                if (operand.car() instanceof IdNode) {
                    innerNode = this.findHashMapData(this.archive.size() - 1, (IdNode) operand.car());
            ...
            case CDR://ListNode의 next부분
                ...
                if (operand.car() instanceof IdNode) {//id 노드일 경우
                    innerNode = this.findHashMapData(this.archive.size() - 1, (IdNode) operand.car());//탐색
                ...
                if (innerNode instanceof ListNode) {//innerNode가 LIstNode인지 판별 해당 노드가 Listnode여야지 car cdr값을 가질 수 있음
                    if (((ListNode) innerNode).car() != null) {
                        ...
                        if (carNode instanceof ListNode && ((ListNode) carNode).car() instanceof QuoteNode) {//반환 값이 List노드의 QuoteNode일 수 있으므로
                            ...
                            carNode = this.findHashMapData(this.archive.size() - 1, (IdNode) carNode);
                    ...
                    if (((ListNode) innerNode).cdr() != null) {//cdr의 이 null이 아닐 경우
                        if (((ListNode) innerNode).cdr().car() instanceof IdNode) {//Id노드 값인 경우 가져온다
                            cdrNode = this.findHashMapData(this.archive.size() - 1, (IdNode) ((ListNode) innerNode).cdr().car());
            ...
            case ATOM_Q://한개의 Node만 존재 여부 판별
                ...
                if (innerNode instanceof ListNode && ((ListNode) innerNode).car() instanceof IdNode) {//id 노드일 경우
                    innerNode = this.findHashMapData(this.archive.size() - 1, (IdNode) ((ListNode) innerNode).car());
            ...
            case EQ_Q:
                ...
                if (firstNode instanceof IdNode) {//id 노드일 경우
                    firstNode = this.findHashMapData(this.archive.size() - 1, (IdNode) firstNode);
                ...
                if (listNodeOfCdr instanceof IdNode) {//두번쨰 값이 id 노드일 경우
                    listNodeOfCdr = this.findHashMapData(this.archive.size() - 1, (IdNode) listNodeOfCdr);
            ...
            case NOT://Not연산에서 List node일 경우가 있고 일반 boolean Node일 경우 가 있다
                ...
                if (resultNode instanceof IdNode) {//id노드일 경우
                    resultNode = this.findHashMapData(this.archive.size() - 1, (IdNode) resultNode);
            ...
            case COND://제일 앞에 True인 Node에 cdr 대해 값을 리턴
                ...
                if (innerNode instanceof IdNode) {//id 노드 일 경우 가져온다
                    innerNode = this.findHashMapData(this.archive.size() - 1, (IdNode) innerNode);
                    if (innerNode == null) {
                ...
                if (innerNode instanceof ListNode) {//listnode 일 경우
                    ...
                    if ((innerNode instanceof BooleanNode && innerNode.toString().equals("#T")) || innerNode instanceof IntNode) {//해당 값이 int노드이거나 boolean노드이 경우
                        if (operand.cdr().car() instanceof ListNode) {
                            ...
                            Node data = this.findHashMapData(this.archive.size() - 1, (IdNode) operand.cdr().car());
                            if (data != null) {
                                return data;
                            }
                            System.out.println("error happen");
            ...
            case DEFINE://정의부분
                ...
                if (operand.cdr().car() instanceof ListNode && ((ListNode) operand.cdr().car()).car() instanceof FunctionNode
                        && ((FunctionNode) ((ListNode) operand.cdr().car()).car()).funcType == FunctionNode.FunctionType.LAMBDA) {
                    this.insertTable(idNodeOfOperandCar, operand.cdr().car());
                    break;
                }
                Node runExprResultData = this.runExpr(operand.cdr().car());
                if (runExprResultData instanceof IdNode) {//해당 값에대해 너헝준게 된다
                    Node data = this.findHashMapData(this.archive.size() - 1, (IdNode) runExprResultData);
                    if (data != null) {
                ...
                return null;
            case LAMBDA:
                HashMap<String, Node> newHashMap = new HashMap<>();
                this.insertNestedVariable(newHashMap, (ListNode) operand.car());//변수 전부 초기화
                this.archive.add(newHashMap);
                Node resultLambdaNode = this.runExpr(operand.cdr().car());//결과 도출
                this.archive.remove(this.archive.size() - 1);
                return resultLambdaNode;
            default:
                break;
        }
        return null;
    ...
    private Node runBinary(ListNode list) {
        ...
        if (secondNode instanceof IdNode) {
            secondNode = this.findHashMapData(this.archive.size() - 1, (IdNode) secondNode);
